...
### 2.1 Phase 0 — Foundation (Enablers; not user-facing)

| # | Feature | Reach | Impact | Conf. | Effort (pw) | RICE | Priority | Dependency |
|---|---|---|---|---|---|---|---|---|
| 0.6 | Exercise Library data file (200+ exercises) | 5 | 3 | 95% | 2 | **7.13** | 🔴 P0 | None — standalone data file |
| 0.7 | Firestore data model + security rules (fitness collections) | 5 | 3 | 95% | 1 | **14.25** | 🔴 P0 | None; blocks 1.1–1.7 |
| 0.8 | types.ts update with fitness types | 5 | 2 | 100% | 0.5 | **20.0** | 🔴 P0 | None; blocks 0.9, 1.1 |
| 0.9 | server.ts: fitness API routes + agent system prompts | 5 | 3 | 90% | 3 | **4.50** | 🔴 P0 | 0.6, 0.7, 0.8 |
| 0.10 | AthenaCodeStore.ts: fitness agent code samples | 3 | 1 | 80% | 1 | **2.40** | 🟡 P1 | 0.9 (code samples reference agent logic) |
| 0.11 | metadata.json + package.json description update | 5 | 1 | 100% | 0.2 | **25.0** | 🔴 P0 | None; documentation only |
| 0.12 | CI/CD deploy.yml | 5 | 2 | 80% | 1 | **8.00** | 🟡 P1 | Required before any production deploy |

**Note:** Phase 0 items are enablers — they have high RICE because they're low-effort and block everything else, not because they're user-facing features.

### 2.2 Phase 1 — Core Coaching Loop (Must Have for MVP)

| # | Feature | Reach | Impact | Conf. | Effort (pw) | RICE | Priority | Dependency |
|---|---|---|---|---|---|---|---|---|
| 1.1 | Fitness onboarding flow (profile capture) | 5 | 3 | 95% | 2 | **7.13** | 🔴 P0 | 0.6, 0.7, 0.8 |
| 1.2 | Workout Generator Agent (weekly plan from profile) | 5 | 3 | 90% | 3 | **4.50** | 🔴 P0 | 0.6, 0.8, 0.9, 1.1 |
| 1.3 | Weekly plan view (7-day grid UI) | 5 | 3 | 95% | 1.5 | **9.50** | 🔴 P0 | 1.2 |
| 1.4 | Active workout session UI (set logging, rest timer) | 5 | 3 | 95% | 2 | **7.13** | 🔴 P0 | 1.3, 0.6 |
| 1.5 | Workout logging to Firestore | 5 | 3 | 95% | 1 | **14.25** | 🔴 P0 | 1.4, 0.7 |
| 1.6 | Plan adaptation (F05: progressive overload + recovery-aware) | 5 | 3 | 85% | 2 | **6.38** | 🔴 P0 | 1.5, 2.4 |
| 1.7 | Progress dashboard (strength trends, frequency charts) | 4 | 2 | 85% | 2 | **3.40** | 🟡 P1 | 1.5 |
...
| # | Feature | Reach | Impact | Conf. | Effort (pw) | RICE | Priority | Dependency |
|---|---|---|---|---|---|---|---|---|
| 2.1 | Coaching Chat Agent + chat UI | 5 | 3 | 90% | 3 | **4.50** | 🔴 P0 | 0.9, 1.1, 1.2 (needs user context) |
| 2.2 | Apple HealthKit integration | 4 | 2 | 75% | 2 | **3.00** | 🟡 P1 | 0.7, 0.9, 2.4 |
| 2.3 | Google Fit integration | 3 | 2 | 70% | 1.5 | **2.80** | 🟡 P1 | 0.7, 0.9, 2.4 |
| 2.4 | Recovery Analyst Agent (score + recommendation) | 4 | 3 | 85% | 1.5 | **6.80** | 🔴 P0 | 0.7, 0.9, 2.2 or 2.3 (needs wearable data for full value) |
| 2.5 | Recovery-based plan adjustment (F05 uses recovery score) | 4 | 3 | 80% | 1 | **9.60** | 🔴 P0 | 2.4, 1.6 |
| 2.6 | Wearable data view dashboards | 3 | 1 | 80% | 1 | **2.40** | 🟡 P2 | 2.2, 2.3 |
| 2.7 | Strava/Garmin/WHOOP/Oura integrations | 2 | 2 | 60% | 2 each | **1.20** | ⚪ P3 | 2.2 or 2.3 (pattern established) |
...
| # | Feature | Reach | Impact | Conf. | Effort (pw) | RICE | Priority | Dependency |
|---|---|---|---|---|---|---|---|---|
| 3.1 | Push notifications + daily digest | 5 | 2 | 80% | 2 | **4.00** | 🔴 P1 | 0.9, 2.2 or 2.3 (digest content needs wearable data), 1.5 |
| 3.2 | Motivation Coach Agent (sentiment + dropout risk) | 4 | 2 | 75% | 1.5 | **4.00** | 🔴 P1 | 2.1 (chat data), 3.3 (check-in data) |
| 3.3 | Check-in flow (post-workout + end-of-day) | 4 | 2 | 80% | 1 | **6.40** | 🔴 P1 | 0.7, 0.9, 3.2 |
| 3.4 | Streaks & consistency tracking | 4 | 1 | 85% | 1 | **3.40** | 🟡 P2 | 1.5 |
| 3.5 | NUX funnel analytics | 2 | 2 | 80% | 1 | **3.20** | 🟡 P2 | 1.1–1.4 (funnel events) |
...
| # | Feature | Reach | Impact | Conf. | Effort (pw) | RICE | Priority | Dependency |
|---|---|---|---|---|---|---|---|---|
| 4.1 | Stripe subscription (3 tiers) | 5 | 3 | 90% | 2 | **6.75** | 🔴 P1 | 1.1–1.6 (need working product before charging) |
| 4.2 | Nutrition Advisor Agent + endpoint | 4 | 2 | 80% | 1.5 | **4.27** | 🟡 P1 | 0.9, 1.1 |
| 4.3 | GLP-1 / special mode programming | 2 | 3 | 70% | 1 | **4.20** | 🟡 P2 | 4.2, 1.2 (adjusted plans), 1.1 (profile flag) |
| 4.4 | Exercise substitution engine (during workout) | 4 | 2 | 85% | 1 | **6.80** | 🔴 P1 | 0.6, 1.4 |
| 4.5 | Text-based form coaching | 3 | 2 | 75% | 1 | **3.75** | 🟡 P2 | 2.1 (chat), 0.6 (exercise library) |
...
├── 0.6 ExerciseLibrary.ts (200+ exercises) [2–3 hours]
├── 0.8 types.ts + fitness types [1 hour]
├── 0.7 Firestore rules update (fitness collections + consent gate) [1 hour]
├── 0.9 server.ts fitness API routes + agent system prompts [4–6 hours]
├── 0.10 AthenaCodeStore.ts fitness agent samples [1 hour]
├── 0.1 PRD.md [done]
├── 0.2 ARCHITECTURE.md [done]
├── 0.3 FEATURES_ROADMAP.md [done]
├── 0.4 IMPACT_ANALYSIS.md [done]
├── 0.5 GAP_ANALYSIS.md [done]
└── README.md + docs/ update [1 hour]
    → COMMIT + PUSH (Phase 0 milestone)

PHASE 1 — CORE COACHING LOOP (Week 3–5)
├── 1.1 Onboarding.tsx → FitnessOnboarding.tsx (profile capture) [3–4 hours]
├── 1.2 F02 Workout Generator Agent logic + endpoint [3–4 hours]
...
PHASE 2 — COACH + WEARABLES (Week 6–8)
...
PHASE 4 — MONETIZATION (Week 12–14)
...
| **Exercise library quality / coverage gaps** | Medium | Medium — bad substitutions or missing exercises hurt UX | Start with 200+ well-known exercises; allow user feedback on substitutions; expand library based on usage patterns |
...
|---|---|---|
| **P0 (MVP spine)** | 0.6, 0.7, 0.8, 0.9, 0.11, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6 | Without these, there is no product. A user cannot onboard, get a plan, do a workout, or see adaptation. |
| **P1 (Differentiators + Engagement)** | 2.1, 2.4, 2.5, 3.1, 3.2, 3.3, 4.1, 4.4, 0.12 | Chat coach + recovery analysis are the competitive moat. Notifications + check-ins drive retention. Stripe unlocks revenue. |
| **P2 (Premium features + Polish)** | 1.7, 2.2, 2.3, 2.6, 3.4, 3.5, 4.2, 4.3, 4.5, 0.10 | Progress dashboards, wearable integrations, nutrition advice, form coaching — valuable but build after core + differentiators are solid. |
| **P3 (Future / Nice-to-have)** | 2.7, 4.1 elite tier details, 5.x | Additional wearable sources, CV form analysis, human coach marketplace, B2B — wait for traction before investing. |
